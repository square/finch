...
func RegisterRDSCA() {
	once.Do(func() {
		...
		if ok := caCertPool.AppendCertsFromPEM(globalBundle); !ok {
			panic(fmt.Errorf("error parsing built-in RDS CA: x509.CertPool.AppendCertsFromPEM returned false"))
		}
		tlsConfig := &tls.Config{RootCAs: caCertPool}
		if err := mysql.RegisterTLSConfig("rds", tlsConfig); err != nil {
			panic(fmt.Errorf("error registering built-in RDS CA: mysql.RegisterTLSConfig: %s", err))
		}
	})
}

// global-bundle.pem downloaded 2024-01-04
// https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.SSL.html
var globalBundle = []byte(`-----BEGIN CERTIFICATE-----
